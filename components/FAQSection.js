...
const FAQSection = () => {
    ...
    return <div className="flex flex-col w-full max-w-[1200px] mx-auto items-center gap-10 py-[100px]">
        ...
        <div className="flex flex-col gap-4 w-full">
            {faqData.map((faq) => (
                <div key={faq.id} className={`flex  gap-4 border border-[#0093E87D] bg-[#217DFE08] backdrop-blur-[70px] rounded-[20px] p-4 ${active === faq.id ? "border-[#21ADFD] bg-[#217DFE21]" : ""}`}>
                    <div onClick={() => handleClick(faq.id)} className={`flex items-center gap-2 w-10 h-10 bg-gray-800 rounded-lg justify-center items-center cursor-pointer ${active === faq.id ? "bg-white text-black" : ""}`}  >
                        <ChevronDownIcon className={`w-6 h-6 text-gray-400 ${active === faq.id ? "rotate-180 transition-transform duration-300" : "transition-transform duration-300"}`} />
                    </div>
